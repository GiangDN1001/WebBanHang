...
<div class="main-content-inner">
    <div class="main-content-wrap">
        ...
        <!-- new-category -->
        <div class="wg-box">
            <form class="form-new-product form-style-1" action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <fieldset>
                    ...
                    @if ($blog->image)
                        ...
                        <img src="{{ asset('uploads/blogs/' . $blog->image) }}" width="120" alt="Blog Image">
                    @endif
                </fieldset>
                @error('image')
                    <span class="alert alert-danger text-center">{{ $message }}</span>
                ...
                <fieldset class="name">
                    ...
                    <div class="select" style="width: 100%">
                        <select name="status" required>
                            <option value="1" {{ old('status', $blog->status) == 1 ? 'selected' : '' }}>On</option>
                            <option value="0" {{ old('status', $blog->status) == 0 ? 'selected' : '' }}>Off</option>
                        </select>
                    </div>
                </fieldset>
                @error('status')
